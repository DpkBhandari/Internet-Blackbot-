...
export default function TrendsPage() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () =>
      (await api.get("/analyses?status=READY&limit=50")).data,
  });
  ...
  const items =
    data?.items?.filter(
      (a: any) => a.status === "READY" && a.keywords?.length,
    ) ?? [];
  ...
  if (!items.length)
    return (
      <div>
        <PageHeader
          title="Viral Trends"
          description="Trending keywords, topics, and emerging narratives."
        />
        <Empty
          icon={<TrendingUp className="h-7 w-7 text-muted" />}
          title="No trend data"
          description="Upload documents to discover trending topics."
        />
      </div>
    );
  ...
  return (
    <div>
      <PageHeader
        title="Viral Trends"
        description="Trending keywords, entities, and emerging narratives from your research corpus."
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-5">
          ...
          <Card>
            <CardBody>
              <CardTitle className="mb-3 flex items-center gap-2">
                <BarChart2 className="h-4 w-4 text-brand" /> Top Keywords by
                Frequency
              </CardTitle>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart
                  data={top20.slice(0, 12)}
                  layout="vertical"
        ...
        >
          <Card>
            <CardBody>
              <CardTitle className="mb-3 flex items-center gap-2">
                <Hash className="h-4 w-4 text-accent" /> Topic Heatmap
              </CardTitle>
              <ResponsiveContainer width="100%" height={280}>
                <Treemap
                  data={treemapData}
      ...
      >
        <Card>
          <div className="px-5 py-3.5 border-b border-border">
            <CardTitle>All Trending Terms</CardTitle>
          </div>
          <CardBody>
            <div className="flex flex-wrap gap-2">
              {sorted.map(([kw, count], i) => {
                const size =
                  count >= 5 ? "text-base" : count >= 3 ? "text-sm" : "text-xs";
                const weight =
                  count >= 5
                    ? "font-bold"
                    : count >= 3
                ...
                return (
                  <motion.span
                    ...
                    className={cn(
                      "px-2.5 py-1 rounded-full border border-brand/20 bg-brand/5 text-brand capitalize cursor-default hover:bg-brand/15 transition-colors",
                      size,
                      weight,
                    )}
                    style={{ opacity }}
