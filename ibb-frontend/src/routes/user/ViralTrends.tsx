...
export default function ViralTrends() {
  const q = useQuery({ queryKey: ["trends"], queryFn: async () => (await api.get<any>(endpoints.analyses.trends)).data });
  return (
    <div>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {d.topTrends.map((t: any) => (
                <Card key={t.id}><CardBody>
                  ...
                  <Badge variant={t.sentiment === "positive" ? "success" : t.sentiment === "negative" ? "danger" : "default"} className="mt-2">{t.sentiment}</Badge>
                </CardBody></Card>
              ))}
            </div>
            <Card><CardBody>
